...
export default {
	...
	methods: {
		addUser: function(){
			...
			var i = this.$store._mutations._addCustomer(this.user);
			if(i == 1) {
				this.chooseIndex = 0;
			}
			this.user = {
		...
		deleteUser: function(index) {
			this.$store._mutations._deleteCustomer(index);
			this.userLength --;
		},
		addItem: function() {
		...
		saveItem: function() {
			...
			this.$store._mutations._modifyCustomer(this.chooseIndex, this.users[this.chooseIndex]);
		},
 		uploadFile: function() {
